...
@section('title', 'Edit Studi Kasus - Admin')

@section('content')
        <!-- Header -->
        <div class="mb-8">
            <a href="{{ route('admin.case-studies.index') }}" class="text-purple-600 hover:text-purple-800 mb-4 inline-block">← Kembali</a>
            <h2 class="text-3xl font-bold text-gray-800 mb-2">Edit Studi Kasus</h2>
        </div>

        <!-- Form -->
        <div class="bg-white rounded-lg shadow border border-gray-200 p-6">
            <form action="{{ route('admin.case-studies.update', $caseStudy->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="number" id="year" name="year" value="{{ old('year', $caseStudy->year) }}" min="2000" max="{{ date('Y') }}" required
                               class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500">
                    ...
                    <div>
                        ...
                        <input type="number" id="order" name="order" value="{{ old('order', $caseStudy->order) }}" min="0"
                               class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500">
                    </div>
                ...
                <div>
                    ...
                    <textarea id="client_context" name="client_context" rows="2"
                              class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500">{{ old('client_context', $caseStudy->client_context) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea id="challenge" name="challenge" rows="4" required
                              class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500">{{ old('challenge', $caseStudy->challenge) }}</textarea>
                ...
                <div>
                    ...
                    <textarea id="solution" name="solution" rows="4" required
                              class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500">{{ old('solution', $caseStudy->solution) }}</textarea>
                ...
                <div>
                    ...
                    <textarea id="outcome" name="outcome" rows="4" required
                              class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500">{{ old('outcome', $caseStudy->outcome) }}</textarea>
                ...
                <div>
                    ...
                    <textarea id="excerpt" name="excerpt" rows="2"
                              class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500">{{ old('excerpt', $caseStudy->excerpt) }}</textarea>
                </div>

                <!-- Tags -->
                <div>
                    <label for="tags" class="block text-sm font-medium text-gray-700 mb-2">Tags (pisahkan dengan koma)</label>
                    <input type="text" id="tags" name="tags" value="{{ old('tags', $caseStudy->tags ? implode(', ', $caseStudy->tags) : '') }}"
                ...
                <div>
                    ...
                    <input type="text" id="visuals" name="visuals" value="{{ old('visuals', $caseStudy->visuals ? implode(', ', $caseStudy->visuals) : '') }}"
