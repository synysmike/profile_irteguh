...
@section('title', 'Pembelian - Keuangan')

@section('keuangan_content')
<div class="flex items-center justify-between mb-6">
    <div>
        <h2 class="text-2xl font-bold text-gray-800">Transaksi Pembelian</h2>
        <p class="text-gray-600 mt-1">Faktur pembelian, PPN masukan. Terkait Data Vendor (Supplier) & Akun Perkiraan.</p>
    </div>
    <button type="button" onclick="openResourceModal('purchaseModal', 'purchases', 'Pembelian')" class="px-4 py-2 bg-purple-600 text-white rounded-md hover:bg-purple-700 transition text-sm font-semibold">
        + Tambah Pembelian
    </button>
</div>
    ...
    <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="divide-y divide-gray-200">
            ...
            <tr id="purchasesRow_{{ $purchase->id }}">
                ...
                <td class="px-4 py-3 text-sm text-right text-gray-600">Rp {{ number_format($purchase->ppn_amount, 0, ',', '.') }}</td>
                ...
                <td class="px-4 py-3 text-right text-sm">
                    ...
                    <button type="button" onclick="openResourceModal('purchaseModal', 'purchases', 'Pembelian', {{ $purchase->id }})" class="text-purple-600 hover:text-purple-800 mr-3">Edit</button>
                    <button type="button" onclick="deleteResource('purchases', {{ $purchase->id }}, 'Pembelian')" class="text-red-600 hover:text-red-800">Hapus</button>
                </td>
            </tr>
            @empty
            <tr>
                <td colspan="7" class="px-4 py-8 text-center text-gray-500">
                    ...
                    <button type="button" onclick="openResourceModal('purchaseModal', 'purchases', 'Pembelian')" class="text-purple-600 hover:text-purple-800 ml-1">Tambah yang pertama</button>
                </td>
            </tr>
            @endforelse
        </tbody>
...
@include('admin.components.modal', ['modalId' => 'purchaseModal', 'title' => 'Tambah Pembelian'])
@endsection
