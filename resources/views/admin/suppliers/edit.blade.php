...
@section('title', 'Edit Tempat Grosir - Admin')

@section('content')
        <div class="mb-8">
            <a href="{{ route('admin.suppliers.index') }}" class="text-purple-600 hover:text-purple-800 mb-4 inline-block">← Kembali</a>
            <h2 class="text-3xl font-bold text-gray-800 mb-2">Edit Tempat Grosir</h2>
        </div>
            ...
            <form action="{{ route('admin.suppliers.update', $supplier->id) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <textarea id="address" name="address" rows="3"
                              class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500">{{ old('address', $supplier->address) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea id="notes" name="notes" rows="3"
                              class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500">{{ old('notes', $supplier->notes) }}</textarea>
                </div>
